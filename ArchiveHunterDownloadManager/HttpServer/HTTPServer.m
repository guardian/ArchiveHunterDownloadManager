...
- (void)start
{
	...
	(id)CFBridgingRelease(addressData);
	...
	close:(BOOL)closeFileHandle
{
	...
	CFDictionaryRemoveValue(incomingRequests, CFBridgingRetain(incomingFileHandle));
}
...
- (void)stop
{
    ...
    NSDictionary *incomingRequestDict = CFBridgingRelease(incomingRequests);
    
	for (NSFileHandle *incomingFileHandle in incomingRequestDict)
	{
...
- (void)receiveIncomingConnectionNotification:(NSNotification *)notification
{
    ...
    if(incomingFileHandle)
	{
		CFDictionaryAddValue(
			incomingRequests,
			CFBridgingRetain(incomingFileHandle),
			(__bridge const void *)((id)CFBridgingRelease(CFHTTPMessageCreateEmpty(kCFAllocatorDefault, TRUE))));
...
- (void)receiveIncomingDataNotification:(NSNotification *)notification
{
	...
	CFHTTPMessageRef incomingRequest =
		(CFHTTPMessageRef)CFDictionaryGetValue(incomingRequests, CFBridgingRetain(incomingFileHandle));
	if (!incomingRequest)
	{
		[self stopReceivingForFileHandle:incomingFileHandle close:YES];
		return;
