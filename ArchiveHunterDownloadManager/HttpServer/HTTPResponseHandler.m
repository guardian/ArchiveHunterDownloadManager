	...
	server:(HTTPServer *)aServer
{
	NSDictionary *requestHeaderFields =
		(NSDictionary *)CFBridgingRelease(CFHTTPMessageCopyAllHeaderFields(aRequest));
	NSURL *requestURL =
		(NSURL *)CFBridgingRelease(CFHTTPMessageCopyRequestURL(aRequest));
	NSString *method =
		(NSString *)CFBridgingRelease(CFHTTPMessageCopyRequestMethod(aRequest));
	...
	server:(HTTPServer *)aServer
{
	...
	if (self != nil)
	{
		request = (CFHTTPMessageRef)CFBridgingRetain((id)CFBridgingRelease(aRequest));
...
- (void)startResponse
{
	...
	CFHTTPMessageSetBody(
		response,
                         (CFDataRef)CFBridgingRetain([[NSString stringWithFormat:
	...
	@try
	{
		[fileHandle writeData:(NSData *)CFBridgingRelease(headerData)];
	}
